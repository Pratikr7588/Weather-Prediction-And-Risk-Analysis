...
def classify_risks_and_mitigations(row):
    ...
    if heatwave_risk in ['High', 'Medium']:
        strategies['Heatwave'] = ["Stay hydrated", "Use air conditioning", "Avoid outdoor activities."]
    if coldwave_risk in ['High', 'Medium']:
        strategies['Coldwave'] = ["Wear warm clothing", "Use heaters indoors", "Minimize outdoor exposure."]
    if flood_risk in ['High', 'Medium']:
        strategies['Flood'] = ["Avoid low-lying areas", "Prepare emergency kits", "Follow alerts."]
    if storm_risk in ['High', 'Medium']:
        strategies['Storm'] = ["Secure outdoor items", "Stay indoors", "Avoid unnecessary travel."]
